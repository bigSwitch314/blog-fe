<template>
  <div>
    ...
    <footer class="footer border-rightleft">
      <Button type="success" class="btn" to="/admin/index/labelAdd">添加标签</Button>
    ...
    export default {
      ...
      methods: {
        ...
        confirmDel: function(id) {
          this.$ajax.post('blog/label/delete', {
            id: id
          }).then(res => {
            this.loading = false;
            if (res.data.errcode === 0) {
        ...
        editLabel (id) {
          this.$router.push('/admin/index/labelEdit?id='+id);
        },
        delLabel (id) { 
        ...
        deleteSeleted () {
          ...
          this.$Modal.confirm({
            title: '提示',
            content: `您确定要删除已选中的标签"<b style="color: red">${names.join(',')}</b>"吗？`,
            onOk: () => {
